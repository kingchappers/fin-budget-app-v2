#!/bin/bash
userPoolId=$(aws ssm get-parameter --name /amplify/d3m9wu6rhd9z99/main/AMPLIFY_COGNITO_USER_POOL_ID | jq '.Parameter.Value')
...
AMPLIFY_TEST=$(aws ssm get-parameter --name AMPLIFY_TEST | jq '.Parameter.Value')